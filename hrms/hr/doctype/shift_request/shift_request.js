	...
	async refresh(frm) {
		...
		if (!frm.is_new() && frm.doc.docstatus === 0) {
			...
			if ((is_approver || is_secondary || is_hr) && !is_employee) {
				["employee", "shift_type", "from_date", "to_date"].forEach(f => {
		...
		_shift_show_approval_tracker(frm);
		_shift_handle_secondary_buttons(frm, is_approver, is_secondary);
	},
...
function _shift_handle_secondary_buttons(frm, is_approver, is_secondary) {
			...
			let d = new frappe.ui.Dialog({
				...
				primary_action(v) {
					...
					frappe.call({
						...
						args: { doctype: "Shift Request", docname: frm.doc.name, reason: v.reason },
						freeze: true,
	...
	if (stage === "Pending Secondary Reporting Approval" && is_secondary) {
		...
		frm.add_custom_button(__("Approve & Submit"), () => {
			frappe.confirm(__("Approve and submit this Shift Request?"), () => {
				frappe.call({
